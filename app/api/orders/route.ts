...
export async function POST(request: Request) {
    ...
    const design = designSlug ? await env.DB.prepare("SELECT id, name, base_price AS basePrice, active FROM designs WHERE slug = ?").bind(designSlug).first<{ id: number; name: string; basePrice: number; active: number }>() : null;
    ...
    `).bind(variantSku, design.id).first<{ sku: string; price: number; stockQuantity: number; materialId: string; sizeId: string; fitId: string; colorId: string }>() : null;
    ...
    try {
        await env.DB.prepare(`INSERT INTO orders (order_code, customer_first_name, customer_last_name, customer_phone, design_id, design_name, collection_slug, material_id, size_id, fit_id, color_id, print_side, placement_id, quantity, unit_price, front_image_key, back_image_key, configuration_json, order_type, variant_sku, total_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
