...
export function ReadyMadeProductConfigurator({ product, variants }: { product: PublicProduct; variants: PublicVariant[] }) {
    ...
    return <div className="grid gap-12 md:grid-cols-[1.15fr_0.85fr] md:gap-20"><section className="relative flex min-h-[600px] items-center justify-center bg-[#efeee9] p-6"><div className="relative aspect-[4/5] w-full max-w-[560px]"><Image src={shirtSource} alt={`${side} view of ${product.name}`} fill priority className="object-contain" />{side === "front" && artworkSource ? <div className={`pointer-events-none absolute z-10 ${placementClasses[placement]}`}><Image src={artworkSource} alt="" fill unoptimized className="object-contain" /></div> : null}</div><div className="absolute bottom-6 left-1/2 flex -translate-x-1/2 rounded-full border border-black/15 bg-white p-1"><button type="button" onClick={() => setSide("front")} className={`rounded-full px-5 py-2 text-sm ${side === "front" ? "bg-black text-white" : ""}`}>{fa ? "جلو" : "Front"}</button><button type="button" onClick={() => setSide("back")} className={`rounded-full px-5 py-2 text-sm ${side === "back" ? "bg-black text-white" : ""}`}>{fa ? "پشت" : "Back"}</button></div></section><div className="md:sticky md:top-12 md:self-start"><h1 className="text-[clamp(3.5rem,7vw,7rem)] font-bold leading-[0.88] tracking-[-0.06em]">{product.name}</h1>{product.description && <p className="mt-7 max-w-md text-base leading-7 text-black/60">{product.description}</p>}<div className="mt-8 space-y-6"><fieldset><legend className="mb-3 text-xs uppercase tracking-[.15em] text-black/45">{fa ? "استایل" : "Fit"}</legend><div className="grid grid-cols-2 gap-2">{fits.map(([id, label]) => <Choice key={id} active={selected.fitId === id} onClick={() => choose("fitId", id)}>{label}</Choice>)}</div></fieldset><fieldset><legend className="mb-3 text-xs uppercase tracking-[.15em] text-black/45">{fa ? "جنس" : "Fabric"}</legend><div className="grid grid-cols-2 gap-2">{materials.map(([id, label]) => <Choice key={id} active={selected.materialId === id} onClick={() => choose("materialId", id)}>{label}</Choice>)}</div></fieldset><fieldset><legend className="mb-3 text-xs uppercase tracking-[.15em] text-black/45">{fa ? "سایز" : "Size"}</legend><div className="flex flex-wrap gap-2">{sizes.map(([id, label]) => <Choice key={id} active={selected.sizeId === id} onClick={() => choose("sizeId", id)}>{label}</Choice>)}</div></fieldset><fieldset><legend className="mb-3 text-xs uppercase tracking-[.15em] text-black/45">{fa ? "رنگ" : "Color"} — {fa ? selected.colorNameFa : selected.colorNameEn}</legend><div className="flex flex-wrap gap-3">{colors.map(([id, color]) => <button type="button" key={id} title={color.name} aria-label={color.name} aria-pressed={selected.colorId === id} onClick={() => choose("colorId", id)} className={`size-10 rounded-full border-2 ${selected.colorId === id ? "scale-110 border-black ring-2 ring-black/20 ring-offset-2" : "border-black/15"}`} style={{ backgroundColor: color.hex }} />)}</div></fieldset></div><div className="mt-8"><div className="mb-4 flex justify-between text-sm"><span>{selected.stockQuantity > 0 ? `${selected.stockQuantity} ${fa ? "عدد موجود" : "available"}` : fa ? "ناموجود" : "Out of stock"}</span><strong>{selected.price.toLocaleString(fa ? "fa-IR" : "en-US")} {fa ? "تومان" : "Toman"}</strong></div>{selected.stockQuantity > 0 ? <OrderCheckout designSlug={product.slug} designName={product.name} collectionSlug={product.collectionSlug} artworkUrl={product.artworkKey} fitId={fit} colorId={selected.colorId} colorName={selected.colorNameFa} materialId={selected.materialId} materialName={selected.materialNameFa} sizeId={selected.sizeId} printSide="front" placementId={placement} unitPrice={selected.price} variantSku={selected.sku} maxQuantity={selected.stockQuantity} /> : <button disabled className="w-full bg-black py-5 text-sm text-white opacity-35">{fa ? "ناموجود" : "Out of stock"}</button>}</div></div></div>;
}
