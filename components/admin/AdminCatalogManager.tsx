...
type Variant = { id: number; designId: number; designName: string; sku: string; price: number; stockQuantity: number; materialId: string; sizeId: string; fitId: string; colorId: string; printMethodId: string; active: number };
type Catalog = { categories: Category[]; designs: Design[]; variants: Variant[]; materials: NamedOption[]; sizes: { id: string; label: string }[]; fits: NamedOption[]; colors: (NamedOption & { hex: string })[]; printMethods: NamedOption[]; placements: string[] };
...
type VariantDraft = { id?: number; designId: string; sku: string; price: string; stockQuantity: string; materialId: string; sizeId: string; fitId: string; colorId: string; printMethodId: string; active: boolean };
...
const emptyVariant: VariantDraft = { designId: "", sku: "", price: "0", stockQuantity: "0", materialId: "", sizeId: "", fitId: "", colorId: "", printMethodId: "dtf", active: true };
...
export function AdminCatalogManager() {
    ...
    const [catalog, setCatalog] = useState<Catalog>({ categories: [], designs: [], variants: [], materials: [], sizes: [], fits: [], colors: [], printMethods: [], placements: [] });
    ...
    async function saveVariant(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const data = await jsonRequest("/api/admin/catalog", { method, body: JSON.stringify({ entity: "variant", ...variantDraft, designId: Number(variantDraft.designId), price: Number(variantDraft.price), stockQuantity: Number(variantDraft.stockQuantity) }) });
            setCatalog(data); setVariantDraft({ ...emptyVariant, designId: variantDraft.designId, materialId: catalog.materials[0]?.id ?? "", sizeId: catalog.sizes[0]?.id ?? "", fitId: catalog.fits[0]?.id ?? "", colorId: catalog.colors[0]?.id ?? "", printMethodId: catalog.printMethods[0]?.id ?? "dtf" }); setMessage("Variant با موفقیت ذخیره شد.");
        } catch (error) { setMessage(error instanceof Error ? error.message : "ذخیره Variant انجام نشد."); }
        finally { setBusy(false); }
    }
    ...
    async function remove(entity: "collection" | "design" | "variant", id: number) {
    ...
    async function toggleVariant(item: Variant) {
        setBusy(true); setMessage("");
        try { setCatalog(await jsonRequest("/api/admin/catalog", { method: "PATCH", body: JSON.stringify({ entity: "variant", ...item, active: !item.active }) })); }
        catch (error) { setMessage(error instanceof Error ? error.message : "تغییر وضعیت انجام نشد."); }
        finally { setBusy(false); }
    }
    ...
    return <main className="min-h-screen bg-[#efeee9] px-5 py-8 md:px-10">
        ...
        <div className="mx-auto mt-8 grid max-w-7xl gap-8 xl:grid-cols-2">
            ...
            <section className="bg-white p-6"><h2 className="text-2xl font-bold">طرح‌ها</h2><form onSubmit={saveDesign} className="mt-5 space-y-3"><div className="grid gap-3 sm:grid-cols-2"><input required placeholder="نام طرح" value={designDraft.name} onChange={(e) => setDesignDraft({ ...designDraft, name: e.target.value })} className="border border-black/20 p-3"/><input required placeholder="slug-design" value={designDraft.slug} onChange={(e) => setDesignDraft({ ...designDraft, slug: e.target.value })} className="border border-black/20 p-3"/><select required value={designDraft.collectionId} onChange={(e) => setDesignDraft({ ...designDraft, collectionId: e.target.value })} className="border border-black/20 p-3"><option value="">انتخاب کالکشن</option>{catalog.categories.map((item) => <option value={item.id} key={item.id}>{item.nameFa}</option>)}</select><input type="number" min="0" placeholder="قیمت پایه" value={designDraft.basePrice} onChange={(e) => setDesignDraft({ ...designDraft, basePrice: e.target.value })} className="border border-black/20 p-3"/></div><textarea placeholder="توضیحات" value={designDraft.description} onChange={(e) => setDesignDraft({ ...designDraft, description: e.target.value })} className="min-h-24 w-full border border-black/20 p-3"/><label className="block border border-dashed border-black/25 p-4 text-sm">آپلود تصویر طرح <input name="artwork" type="file" accept="image/png,image/jpeg,image/webp" className="mt-2 block w-full"/></label><fieldset><legend className="mb-2 text-sm">Placementهای مجاز</legend><div className="flex flex-wrap gap-2">{catalog.placements.map((placement) => <label key={placement} className="flex items-center gap-2 border border-black/15 px-3 py-2 text-xs"><input type="checkbox" checked={designDraft.placements.includes(placement)} onChange={(e) => setDesignDraft({ ...designDraft, placements: e.target.checked ? [...designDraft.placements, placement] : designDraft.placements.filter((item) => item !== placement) })}/>{placementLabels[placement]}</label>)}</div></fieldset><button disabled={busy} className="w-full bg-black p-4 text-white">{designDraft.id ? "ذخیره تغییرات طرح" : "افزودن طرح"}</button>{designDraft.id ? <button type="button" onClick={() => setDesignDraft({ ...emptyDesign, collectionId: String(catalog.categories[0]?.id ?? "") })} className="w-full border border-black/20 p-3">انصراف از ویرایش</button> : null}</form>
                <div className="mt-7 space-y-3">{catalog.designs.map((item) => <article key={item.id} className="flex gap-3 border border-black/10 p-3">{item.artworkKey?.startsWith("https://") ? <div className="relative size-20 shrink-0 bg-[#efeee9]"><Image src={item.artworkKey} alt="" fill unoptimized className="object-contain"/></div> : <div className="flex size-20 shrink-0 items-center justify-center bg-[#efeee9] text-xs text-black/35">No image</div>}<div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className={`size-2 rounded-full ${item.active ? "bg-green-600" : "bg-black/20"}`}/><h3 className="truncate font-medium">{item.name}</h3></div><p className="mt-1 text-xs text-black/45">{item.collectionName} · {item.placements?.split(",").map((p) => placementLabels[p]).join(", ")}</p><div className="mt-3 flex gap-3"><button onClick={() => setDesignDraft({ id: item.id, slug: item.slug, name: item.name, description: item.description, basePrice: String(item.basePrice), artworkKey: item.artworkKey ?? "", collectionId: String(item.collectionId ?? ""), placements: item.placements?.split(",") ?? ["center"], active: Boolean(item.active) })} className="text-xs underline">ویرایش</button><button onClick={() => toggle("design", item)} className="text-xs underline">{item.active ? "غیرفعال" : "فعال"}</button><button onClick={() => remove("design", item.id)} className="text-xs text-red-700 underline">حذف</button></div></div></article>)}</div>
            </section>
            <section className="bg-white p-6 xl:col-span-2"><h2 className="text-2xl font-bold">تنوع‌های قابل فروش</h2><p className="mt-2 text-sm text-black/45">برای هر ترکیب رنگ، استایل، جنس و سایز یک SKU، قیمت و موجودی تعریف کنید.</p><form onSubmit={saveVariant} className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-5"><select required value={variantDraft.designId} onChange={(e) => setVariantDraft({ ...variantDraft, designId: e.target.value })} className="border border-black/20 p-3"><option value="">انتخاب طرح</option>{catalog.designs.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><input required placeholder="SKU یکتا" value={variantDraft.sku} onChange={(e) => setVariantDraft({ ...variantDraft, sku: e.target.value })} className="border border-black/20 p-3"/><select required value={variantDraft.fitId} onChange={(e) => setVariantDraft({ ...variantDraft, fitId: e.target.value })} className="border border-black/20 p-3"><option value="">استایل</option>{catalog.fits.map((item) => <option value={item.id} key={item.id}>{item.nameFa}</option>)}</select><select required value={variantDraft.materialId} onChange={(e) => setVariantDraft({ ...variantDraft, materialId: e.target.value })} className="border border-black/20 p-3"><option value="">جنس</option>{catalog.materials.map((item) => <option value={item.id} key={item.id}>{item.nameFa}</option>)}</select><select required value={variantDraft.sizeId} onChange={(e) => setVariantDraft({ ...variantDraft, sizeId: e.target.value })} className="border border-black/20 p-3"><option value="">سایز</option>{catalog.sizes.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select><select required value={variantDraft.colorId} onChange={(e) => setVariantDraft({ ...variantDraft, colorId: e.target.value })} className="border border-black/20 p-3"><option value="">رنگ</option>{catalog.colors.map((item) => <option value={item.id} key={item.id}>{item.nameFa}</option>)}</select><select required value={variantDraft.printMethodId} onChange={(e) => setVariantDraft({ ...variantDraft, printMethodId: e.target.value })} className="border border-black/20 p-3"><option value="">نوع چاپ</option>{catalog.printMethods.map((item) => <option value={item.id} key={item.id}>{item.nameFa}</option>)}</select><input required type="number" min="0" placeholder="قیمت تومان" value={variantDraft.price} onChange={(e) => setVariantDraft({ ...variantDraft, price: e.target.value })} className="border border-black/20 p-3"/><input required type="number" min="0" placeholder="موجودی" value={variantDraft.stockQuantity} onChange={(e) => setVariantDraft({ ...variantDraft, stockQuantity: e.target.value })} className="border border-black/20 p-3"/><button disabled={busy} className="bg-black p-3 text-white">{variantDraft.id ? "ذخیره تغییرات" : "افزودن Variant"}</button></form>{variantDraft.id ? <button type="button" onClick={() => setVariantDraft(emptyVariant)} className="mt-3 border border-black/20 px-4 py-2 text-sm">انصراف از ویرایش</button> : null}<div className="mt-6 overflow-x-auto"><table className="w-full min-w-[900px] text-right text-sm"><thead><tr className="border-b border-black/15 text-black/45"><th className="p-3">طرح / SKU</th><th className="p-3">استایل</th><th className="p-3">جنس</th><th className="p-3">سایز</th><th className="p-3">رنگ</th><th className="p-3">قیمت</th><th className="p-3">موجودی</th><th className="p-3">عملیات</th></tr></thead><tbody>{catalog.variants.map((item) => <tr key={item.id} className="border-b border-black/10"><td className="p-3"><strong>{item.designName}</strong><br/><span className="text-xs text-black/40">{item.sku}</span></td><td className="p-3">{catalog.fits.find((option) => option.id === item.fitId)?.nameFa}</td><td className="p-3">{catalog.materials.find((option) => option.id === item.materialId)?.nameFa}</td><td className="p-3">{catalog.sizes.find((option) => option.id === item.sizeId)?.label}</td><td className="p-3">{catalog.colors.find((option) => option.id === item.colorId)?.nameFa}</td><td className="p-3">{item.price.toLocaleString("fa-IR")}</td><td className="p-3">{item.stockQuantity}</td><td className="p-3"><div className="flex gap-3"><button onClick={() => setVariantDraft({ id: item.id, designId: String(item.designId), sku: item.sku, price: String(item.price), stockQuantity: String(item.stockQuantity), materialId: item.materialId, sizeId: item.sizeId, fitId: item.fitId, colorId: item.colorId, printMethodId: item.printMethodId, active: Boolean(item.active) })} className="text-xs underline">ویرایش</button><button onClick={() => toggleVariant(item)} className="text-xs underline">{item.active ? "غیرفعال" : "فعال"}</button><button onClick={() => remove("variant", item.id)} className="text-xs text-red-700 underline">حذف</button></div></td></tr>)}</tbody></table>{!catalog.variants.length && <p className="py-8 text-center text-sm text-black/45">هنوز Variantی تعریف نشده است.</p>}</div></section>
        </div>
    </main>;
}
