...
export function OrderCheckout(props: Props) {
    ...
    return <div><div className="mb-4 flex items-center justify-between border-y border-black/15 py-4 text-sm"><label htmlFor="order-quantity">{fa ? "تعداد" : "Quantity"}</label><div className="flex items-center gap-3"><input id="order-quantity" name="quantity" type="number" min="1" max={maxQuantity} value={quantity} onChange={(event) => setQuantity(Math.min(maxQuantity, Math.max(1, Number(event.target.value) || 1)))} className="w-20 border border-black/15 p-2 text-center"/><strong>{(props.unitPrice * quantity).toLocaleString(fa ? "fa-IR" : "en-US")} {fa ? "تومان" : "Toman"}</strong></div></div><button type="button" onClick={() => setOpen((value) => !value)} className="w-full bg-[var(--color-primary)] py-5 text-sm font-medium transition hover:bg-black hover:text-white">{open ? (fa ? "بستن فرم سفارش" : "Close order form") : (fa ? "ثبت سفارش" : "Place order")}</button>{open ? <form onSubmit={submit} dir={fa ? "rtl" : "ltr"} className="mt-4 space-y-3 border border-black/15 p-4"><div className="grid grid-cols-2 gap-3"><input name="firstName" required maxLength={60} placeholder={fa ? "نام" : "First name"} className="border border-black/20 p-3 text-sm"/><input name="lastName" required maxLength={80} placeholder={fa ? "نام خانوادگی" : "Last name"} className="border border-black/20 p-3 text-sm"/></div><input name="phone" required inputMode="tel" pattern="[+0-9 ()-]{8,20}" placeholder={fa ? "شماره تماس" : "Phone number"} className="w-full border border-black/20 p-3 text-sm"/><input name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true"/><p className="text-xs leading-5 text-black/45">{fa ? "با ثبت سفارش، مشخصات تماس و تصاویر نهایی برای پیگیری سفارش ذخیره می‌شوند." : "Your contact details and final previews are saved to process the order."}</p><button disabled={busy} className="w-full bg-black py-4 text-sm text-white disabled:opacity-50">{busy ? (fa ? "در حال ساخت تصاویر…" : "Creating previews…") : (fa ? "تأیید و ثبت سفارش" : "Confirm order")}</button>{message ? <p className="text-sm text-red-700">{message}</p> : null}</form> : null}</div>;
}
