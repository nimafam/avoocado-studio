...
const fits: { id: Fit; label: string; source: string }[] = [
    { id: "loose", label: "Loose fit", source: "/models/tshirt-turntable-loose.png" },
    { id: "boxy", label: "Boxy fit", source: "/models/tshirt-turntable-boxy.png" },
];
...
const placements: { id: Placement; label: string; className: string }[] = [
    { id: "center", label: "Center", className: "top-[39%] left-1/2 size-[15%] -translate-x-1/2" },
    { id: "chest", label: "Left chest", className: "top-[35%] left-[41%] size-[8%]" },
    { id: "large", label: "Large center", className: "top-[34%] left-1/2 size-[23%] -translate-x-1/2" },
    { id: "lower", label: "Lower center", className: "top-[53%] left-1/2 size-[13%] -translate-x-1/2" },
];

const frameLabels = ["Front", "Front ¾", "Right", "Back ¾", "Back", "Back ¾", "Left", "Front ¾"];
...
export function TShirtTurntable() {
    ...
    const designPerspective = frame === 1 || frame === 3 ? "translateX(10%) skewY(2deg) scaleX(.72)" : frame === 7 || frame === 5 ? "translateX(-10%) skewY(-2deg) scaleX(.72)" : "none";

    function handlePointerMove(clientX: number) {
        if (!dragStart.current) return;
        const delta = Math.round((dragStart.current.x - clientX) / 55);
        setFrame(((dragStart.current.frame + delta) % 8 + 8) % 8);
    }
    ...
    return (
        <div className="grid min-h-[calc(100svh-112px)] lg:grid-cols-[1.35fr_0.65fr]">
            <section className="relative flex min-h-[620px] touch-none flex-col items-center justify-center overflow-hidden bg-[#efeee9] px-5 py-10 md:min-h-[760px] md:px-12">
                <div className="absolute left-6 top-6 z-20 flex gap-2 text-xs font-medium uppercase tracking-[0.15em] text-black/45"><span>360° preview</span><span>•</span><span>{frameLabels[frame]}</span></div>
                <div
                    className="relative aspect-[3/2] w-full max-w-[850px] cursor-ew-resize select-none overflow-hidden bg-white"
                    onPointerDown={(event) => { dragStart.current = { x: event.clientX, frame }; event.currentTarget.setPointerCapture(event.pointerId); }}
                    ...
                    aria-label={`${activeFit.label}, ${frameLabels[frame]} view`}
                >
                    <img
                        src={activeFit.source}
                        alt=""
                        draggable={false}
                        className="absolute inset-y-0 left-0 h-full w-[800%] max-w-none object-fill transition-[filter] duration-300"
                        style={{ transform: `translateX(-${frame * 12.5}%)`, filter: color.filter }}
                    />
                    <div className={`pointer-events-none absolute flex items-center justify-center rounded-full font-bold leading-none text-black shadow-md transition-opacity duration-200 ${activePlacement.className} ${designVisible ? "opacity-100" : "opacity-0"}`} style={{ backgroundColor: design.color, transform: designPerspective }}>
                        <span className={placement === "chest" ? "text-[clamp(.75rem,2vw,1.5rem)]" : "text-[clamp(1.5rem,5vw,4.5rem)]"}>{design.mark}</span>
                    </div>
                </div>
                <div className="mt-7 w-full max-w-[680px]">
                    <input aria-label="Rotate T-shirt" type="range" min="0" max="7" step="1" value={frame} onChange={(event) => setFrame(Number(event.target.value))} className="w-full accent-black" />
                    <div className="mt-3 flex justify-between text-[10px] font-medium uppercase tracking-[0.14em] text-black/40"><span>Drag left</span><span>{frame + 1} / 8</span><span>Drag right</span></div>
                </div>
            </section>
            ...
            <aside className="border-l border-black/15 bg-[var(--color-background)] px-6 py-10 md:px-10 lg:max-h-[calc(100svh-112px)] lg:overflow-y-auto">
                <div className="mb-9"><span className="text-xs font-medium uppercase tracking-[0.18em] text-[var(--color-primary)]">Avoocado Custom Lab</span><h1 className="mt-4 text-4xl font-bold tracking-[-0.05em] md:text-5xl">Build your tee.</h1><p className="mt-4 text-sm leading-6 text-black/50">Drag the realistic preview to inspect every angle, then choose the fit, color and print position.</p></div>
                <div className="space-y-8">
                    ...
                    <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">04 / Print side</legend><div className="grid grid-cols-2 gap-2"><Option active={printSide === "front"} onClick={() => { setPrintSide("front"); setFrame(0); }}>Front</Option><Option active={printSide === "back"} onClick={() => { setPrintSide("back"); setFrame(4); }}>Back</Option></div></fieldset>
