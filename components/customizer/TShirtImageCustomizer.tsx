...
export function TShirtImageCustomizer() {
    ...
    return <div className="grid min-h-[calc(100svh-112px)] lg:grid-cols-[1.35fr_0.65fr]">
        <section className="relative flex min-h-[620px] items-center justify-center bg-[#efeee9] px-6 pb-28 pt-12 md:min-h-[760px]">
            <div className="absolute left-6 top-6 text-xs font-medium uppercase tracking-[0.15em] text-black/45">Product preview · {fit} fit · viewing {viewSide}</div>
            ...
            <div className="absolute bottom-8 left-1/2 flex -translate-x-1/2 overflow-hidden rounded-full border border-black/15 bg-white p-1 shadow-sm" aria-label="T-shirt view"><button type="button" onClick={() => setViewSide("front")} className={`min-w-28 rounded-full px-6 py-3 text-sm transition ${viewSide === "front" ? "bg-black text-white" : "hover:bg-black/5"}`}>View front</button><button type="button" onClick={() => setViewSide("back")} className={`min-w-28 rounded-full px-6 py-3 text-sm transition ${viewSide === "back" ? "bg-black text-white" : "hover:bg-black/5"}`}>View back</button></div>
        </section>
        <aside className="border-l border-black/15 bg-[var(--color-background)] px-6 py-10 md:px-10 lg:max-h-[calc(100svh-112px)] lg:overflow-y-auto">
            <div className="mb-9"><span className="text-xs font-medium uppercase tracking-[0.18em] text-[var(--color-primary)]">Avoocado Custom Lab</span><h1 className="mt-4 text-4xl font-bold tracking-[-0.05em] md:text-5xl">Build your tee.</h1><p className="mt-4 text-sm leading-6 text-black/50">Select a collection, choose its artwork, then configure your T-shirt and print.</p></div>
            <div className="space-y-8">
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">01 / Collection</legend><select value={collection} onChange={(event) => selectCollection(event.target.value)} className="w-full border border-black/20 bg-transparent px-4 py-3 text-sm outline-none transition focus:border-black">{categories.map((item) => <option value={item.slug} key={item.slug}>{item.nameEn}</option>)}</select></fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">02 / Artwork</legend>{collectionArtworks.length ? <div className="grid grid-cols-2 gap-2">{collectionArtworks.map((item) => <button type="button" aria-pressed={selectedArtwork?.slug === item.slug} onClick={() => selectArtwork(item)} className={`flex min-h-20 items-center gap-3 border p-3 text-left text-sm transition ${selectedArtwork?.slug === item.slug ? "border-black bg-black text-white" : "border-black/20 hover:border-black"}`} key={item.slug}>{item.artworkUrl ? <span className="relative size-10 shrink-0 overflow-hidden bg-white"><Image src={item.artworkUrl} alt="" fill unoptimized className="object-contain"/></span> : <span className="flex size-10 shrink-0 items-center justify-center rounded-full font-bold text-black" style={{ backgroundColor: item.tone }}>{item.mark}</span>}<span>{item.name}</span></button>)}</div> : <p className="border border-dashed border-black/20 p-4 text-sm leading-6 text-black/45">No active artwork is available in this collection.</p>}</fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">03 / T-shirt style</legend><div className="grid grid-cols-2 gap-2"><Option active={fit === "loose"} onClick={() => setFit("loose")}>Loose fit</Option><Option active={fit === "boxy"} onClick={() => setFit("boxy")}>Boxy fit</Option></div></fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">04 / Fabric</legend><div className="grid grid-cols-2 gap-2">{shirtMaterials.map((item, index) => <Option active={material === index} onClick={() => setMaterial(index)} key={item.id}>{item.labelEn}</Option>)}</div></fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">05 / Size</legend><div className="grid grid-cols-4 gap-2">{shirtSizes.map((item, index) => <Option active={size === index} onClick={() => setSize(index)} key={item}>{item}</Option>)}</div></fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">06 / Color — {colors[color][0]}</legend><div className="flex flex-wrap gap-3">{colors.map((item, index) => <button type="button" onClick={() => setColor(index)} aria-label={`Choose ${item[0]}`} aria-pressed={color === index} className={`size-10 rounded-full border-2 transition-transform hover:scale-110 ${color === index ? "scale-110 border-black ring-2 ring-black/20 ring-offset-2" : "border-black/15"}`} style={{ backgroundColor: item[1] }} key={item[3]} />)}</div></fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">07 / Print side</legend><div className="grid grid-cols-2 gap-2"><Option active={printSide === "front"} onClick={() => setPrintSide("front")}>Print on front</Option><Option active={printSide === "back"} onClick={() => setPrintSide("back")}>Print on back</Option></div></fieldset>
                <fieldset><legend className="mb-3 text-xs font-medium uppercase tracking-[0.15em] text-black/45">08 / Placement</legend>{allowedPlacements.length ? <div className="grid grid-cols-2 gap-2">{allowedPlacements.map((item) => <Option active={placement === item[0]} onClick={() => setPlacement(item[0])} key={item[0]}>{item[1]}</Option>)}</div> : <p className="text-sm text-black/45">Choose an artwork to see its available placements.</p>}</fieldset>
            </div>
            <div className="mt-9 border-t border-black/15 pt-6"><div className="mb-5 flex justify-between text-sm"><span>{selectedArtwork?.name ?? "Choose an artwork"}</span><span className="font-medium">$46</span></div>{selectedArtwork ? <OrderCheckout designSlug={selectedArtwork.slug} designName={selectedArtwork.name} collectionSlug={collection} artworkUrl={selectedArtwork.artworkUrl} artworkMark={selectedArtwork.mark} artworkTone={selectedArtwork.tone} fitId={fit} colorId={colors[color][3]} colorName={colors[color][0]} materialId={shirtMaterials[material].id} materialName={shirtMaterials[material].labelFa} sizeId={shirtSizes[size]} printSide={printSide} placementId={placement} unitPrice={46} /> : <button type="button" disabled className="w-full bg-[var(--color-primary)] py-5 text-sm font-medium opacity-40">ابتدا یک طرح انتخاب کنید</button>}</div>
        </aside>
    </div>;
}
